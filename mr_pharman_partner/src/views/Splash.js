...
import { bold, white_logo, api_url, app_settings } from "../config/Constants";
  ...
  const get_app_settings = async () => {
    ...
    .then(async (response) => {
      if (5 > app_version_code) {
        navigate_update_app('https://play.google.com/store/apps/details?id=com.mrpharman.partner');
...
const mapDispatchToProps = (dispatch) => ({
  updatePartnerProfilePicture: (data) =>
    dispatch(updatePartnerProfilePicture(data)),
});
